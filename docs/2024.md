...
## 通过vitepress快速搭建笔记（博客）中心

参考官网[vitepress-getstart](https://vitepress.dev/zh/guide/getting-started) 搭建即可。


## Vm Fusion虚拟机设置桥接网络

背景：虚拟机部署的jenkins无法发布给局域网的其他同事使用

需求：与本机、局域网互通

核心点： Rocky9 Linux已经将网络管理替换为NetworkManger工具，弃用了ifconfig的配置
...
直播目前应该的场景十分广泛，本章主要讨论直播流服务的搭建和使用。

直播的数据流：

![20241122094224](https://stephensunny.oss-cn-shenzhen.aliyuncs.com//springboot/20241122094224.png)


### srs

srs介绍参考：https://ossrs.net/lts/zh-cn/

#### 搭建srs

我们可以通过docker的方式，轻易地搭建起srs服务
...
![20241122091912](https://stephensunny.oss-cn-shenzhen.aliyuncs.com//springboot/20241122091912.png)
...
免费视频资源下载： https://sample-videos.com/index.php#sample-flv-video

PS: MacOS Apple芯片用户，可以用`homebrew install ffmpeg`

```shell
ffmpeg -re -i ./SampleVideo_1280x720_1mb.flv -c copy -f flv -y rtmp://192.168.0.104/live/livestream
...
基于rtmp协议的流数据，可以直接使用VCL播放器播放：

![image-20241121130739226](https://stephensunny.oss-cn-shenzhen.aliyuncs.com//springboot/image-20241121130739226.png)

<br/>

- 网页播放

我们比较多场景，是基于浏览器端进行播放的，这时候，需要将rtmp转换成webtrc进行播放，并且使用支持webrtc的播放器进行播放：

srs已经自带该功能，我们可以访问一下地址：

http://hostIP:8080/players/rtc_player.html

点击“播放视频”即可拉取到刚刚ffmpeg的视频流

![image-20241121130806192](https://stephensunny.oss-cn-shenzhen.aliyuncs.com//springboot/image-20241121130806192.png)
